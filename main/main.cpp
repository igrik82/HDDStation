...
void http_server(void *pvParameter) {
  // Get NVS object
  Nvs_NS::Nvs *nvs = static_cast<Nvs_NS::Nvs *>(pvParameter);
  Http_NS::HttpServer server(nvs);
...
void wifi_connection(void *pvParameter) {
  ...
  // NOTE: In esp8266 std::string works not guaranteed.
  // Because of that we use char array
  char wifi_ssid[18] = {0};
  char wifi_password[24] = {0};
  char wifi_ssid_def[] = WIFI_SSID;
  char wifi_password_def[] = WIFI_PASSWORD;

  nvs->read_str(WIFI_SSID_KEY, wifi_ssid, wifi_ssid_def);
  nvs->read_str(WIFI_PASSWORD_KEY, wifi_password, wifi_password_def);
  ...
  for (;;) {
    ...
    for (uint8_t i = 0; i < SENSOR_COUNT; i++) {
      ...
      if (value_index[i] == 0) {
        ...
        float filtered_temp =
            process_sensor_values(sensor_values[i][0], // Oldest reading
        ...
        // Send to mqtt task
        if (xQueueSend(temperature_queue, &sensor_data, portMAX_DELAY) !=
            pdPASS) {
          ESP_LOGE("DS18B20", "Failed to send data to queue from main.cpp");
        }
        // Send to fan control task
        if (xQueueSend(temperature_queue_PWM, &sensor_data, portMAX_DELAY) !=
            pdPASS) {
          ESP_LOGE("DS18B20", "Failed to send data to queue from main.cpp");
        }
      }
...
void checkStackUsage(void *pvParameter) {
  {
    for (;;) {
      UBaseType_t highWaterMark =
          uxTaskGetStackHighWaterMark(http_server_handle);
...
extern "C" void app_main(void) {
  ...
  xTaskCreate(&wifi_connection, "Wifi", STACK_TASK_SIZE, &nvs, 5,
              &wifi_connection_handle);

  xTaskCreate(&mqtt_connection, "Mqtt", STACK_TASK_SIZE, &nvs, 5,
